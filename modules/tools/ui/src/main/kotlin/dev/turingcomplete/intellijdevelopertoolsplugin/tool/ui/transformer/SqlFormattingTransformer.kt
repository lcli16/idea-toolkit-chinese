...
class SqlFormattingTransformer(
  ...
  override fun Panel.buildSourceTopConfigurationUi() {
    row { comboBox(Dialect.entries).label("方言:").bindItem(dialect) }.layout(RowLayout.PARENT_GRID)
    ...
    row { checkBox("将关键字转换为大写").bindSelected(uppercase) }.layout(RowLayout.PARENT_GRID)
  }
  ...
  class Factory : DeveloperUiToolFactory<SqlFormattingTransformer> {
    ...
    override fun getDeveloperUiToolPresentation() =
      DeveloperUiToolPresentation(menuTitle = "SQL 格式化", contentTitle = "SQL 格式化")
