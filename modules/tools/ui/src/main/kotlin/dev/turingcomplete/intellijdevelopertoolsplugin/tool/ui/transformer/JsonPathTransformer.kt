...
class JsonPathTransformer(
  ...
  override fun Panel.buildMiddleConfigurationUi() {
    row {
      expandableTextField()
        .bindText(queryText)
        .label("JSON 路径:", LabelPosition.TOP)
  ...
  override fun Row.buildAdditionalActionsUi() {
    checkBox("设置结果格式").bindSelected(formatResult).whenStateChangedFromUi {
    ...
    try {
      ...
      if (formatResult.get()) {
        ...
        PsiManager.getInstance(project!!).findFile(workingVirtualFile)?.let { workingPsiFile ->
          ...
          processor.run()
        } ?: error("snh：无法获取 的 PSI 文件 `LightVirtualFile`")
      } else {
  ...
  class Factory : DeveloperUiToolFactory<JsonPathTransformer> {
    ...
    override fun getDeveloperUiToolPresentation() =
      DeveloperUiToolPresentation(menuTitle = "JSON 路径", contentTitle = "JSON 路径转换器")
  ...
  private class ShowOperatorsHelpPopup(val helpButton: () -> JComponent) :
    DumbAwareAction("操作帮助", null, AllIcons.General.ContextHelp), PopupAction {
