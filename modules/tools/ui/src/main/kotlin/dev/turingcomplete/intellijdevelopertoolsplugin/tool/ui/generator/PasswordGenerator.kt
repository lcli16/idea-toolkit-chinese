...
class PasswordGenerator(
  ...
  override fun Panel.buildConfigurationUi() {
    rowsRange {
      row {
          textField().label("长度:").validateLongValue(LongRange(1, 100)).bindIntTextImproved(length)
        }
  ...
  private enum class LettersMode(val title: String) {
    ...
    NONE("无"),
    ASCII_ALPHABET("ASCII 字母"),
    ASCII_ALPHABET_ONLY_LOWERCASE("ASCII 字母 - 仅限小写"),
    ASCII_ALPHABET_ONLY_UPPERCASE("ASCII 字母 - 仅限大写");
  ...
  class Factory : DeveloperUiToolFactory<PasswordGenerator> {
    ...
    override fun getDeveloperUiToolPresentation() =
      DeveloperUiToolPresentation(menuTitle = "密码生成器", contentTitle = "密码生成器")
